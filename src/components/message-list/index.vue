<template>
  <el-scrollbar
    class="message-group-box"
    ref="refScrollbar"
    @scroll="scrollHandle"
  >
    <div ref="refInner">
      <el-icon v-show="loading"><loading /></el-icon>
      <!-- <span class="tips" v-show="!loading && list.length >= page.total"
        >暂无更多</span
      > -->
      <message
        v-show="!loading"
  ...
  setup() {
    ...
    const getMessage = async (clear) => {
      ...
      const params = {
        ...
        times:
          store.state.message.privateMessages.length && !clear
            ? store.state.message.privateMessages[
                store.state.message.privateMessages.length - 1
              ].created_at
            : "",
      };
      const r = await store.dispatch("message/getPrivateMessages", {
        params,
        clear,
      });
      loading.value = false;
    ...
    const scrollHandle = ({ scrollTop }) => {
      ...
      if (!loading.value && scrollTop === 0 && list.value.length < page.total) {
        ...
        getMessage(false).then((r) => {
          if (r) {
            nextTick(() => {
              refScrollbar.value.setScrollTop(
                refInner.value.clientHeight - height
              );
            });
          }
        });
